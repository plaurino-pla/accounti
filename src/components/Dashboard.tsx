...
const Dashboard: React.FC<DashboardProps> = ({ user, onSwitchToAdmin, isImpersonating = false }) => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50">
      {/* Modern Header */}
      <header className={`bg-white/80 backdrop-blur-md border-b border-white/20 sticky z-50 ${isImpersonating ? 'top-10' : 'top-0'}`}>
              ...
              <Logo size="sm" showTagline={false} color="dark" />
                ...
                >
                  <div className="flex items-center space-x-2">
                    ...
                    <span>{isActivityFeedVisible ? 'Hide' : 'Show'} Activity</span>
                    {activities.length > 0 && (
      ...
      {/* Floating Activity Button (when sidebar is closed) */}
      {!isActivityFeedVisible && (
        <button
          onClick={showActivityFeed}
          className="fixed bottom-6 right-6 bg-blue-600 text-white p-4 rounded-full shadow-lg hover:bg-blue-700 transition-all duration-200 z-30 group"
        >
          <div className="flex items-center space-x-2">
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 10V3L4 14h7v7l9-11h-7z" />
          ...
          <div className="absolute right-full mr-3 bg-gray-900 text-white text-sm px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap">
            Show Activity Feed
          </div>
        </button>
      )}
