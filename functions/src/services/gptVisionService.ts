...
9. ALWAYS return valid JSON`
  ...
  // Process invoice with ChatGPT using Vision API for images
  async processInvoiceWithVision(buffer: Buffer, filename: string): Promise<GPTExtractedData> {
    console.log('=== CHATGPT VISION PROCESSING START ===');
    console.log('Filename:', filename);
    console.log('Buffer size:', buffer.length, 'bytes');
    ...
    try {
      ...
      console.log('Sending to ChatGPT Vision API...');
      
      // Send to ChatGPT with Vision API
      const response = await this.openai.chat.completions.create({
        model: 'gpt-4o-mini',
        messages: [
          {
...
9. ALWAYS return valid JSON`
          },
          {
            ...
            content: [
              {
                type: 'text',
                text: 'Please extract invoice data from this image. Return only valid JSON.'
              },
              {
                type: 'image_url',
                image_url: {
      ...
      console.log('ChatGPT Vision response received');
      ...
      console.log('ChatGPT Vision raw response:', content);
      ...
      try {
        ...
        console.error('Failed to parse ChatGPT Vision response:', parseError);
        console.log('Raw response was:', content);
        // Create fallback data
        extractedData = {
          isInvoice: false,
      ...
      console.log('=== CHATGPT VISION EXTRACTION COMPLETE ===');
      console.log('Extracted data:', cleanData);

      return cleanData;

    } catch (error) {
      console.error('=== CHATGPT VISION PROCESSING FAILED ===');
      console.error('Error:', error);
  ...
  async extractInvoiceDataFromImage(buffer: Buffer): Promise<GPTExtractedData> {
    return this.processInvoiceWithVision(buffer, 'unknown.jpg');
  }
} 